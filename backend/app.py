...
@app.route('/match_stats/<int:match_id>', methods=['GET'])
def get_match_stats(match_id):
    from football_crud import get_match_statistics_with_goals
    return jsonify(get_match_statistics_with_goals(match_id))
...
def get_player_by_id(player_id):
    ...
    cursor.execute("""
        SELECT 
            p.id, p.name, p.team_id, p.photo, p.height, p.weight, 
            p.age, p.biography, p.games, p.minutes, p.goals, p.assists,
            t.logo AS team_logo
        FROM players p
